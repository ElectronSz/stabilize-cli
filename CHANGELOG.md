...
- **db:backup** command: Backup the database to a timestamped file. SQLite databases are copied directly; PostgreSQL/MySQL export to JSON format. Supports `--output` flag for custom backup directory.
- **db:restore** command: Restore the database from a backup file (.db for SQLite, .json for others). Includes `--force` flag to skip confirmation.
- **generate:api** command: Generate a full REST API scaffold from a model with list, getOne, create, update, and remove routes. Supports `--prefix` flag for custom route prefix.
- **migrate:fresh** command: Drop all tables and re-run all migrations without seeding. Supports `--force` flag.
- **db:size** command: Show database and table size statistics including file size (SQLite), row counts, and table sizes (Postgres/MySQL).
- Improved banner display with box-drawing characters.
- Expanded `info` command to show memory usage, PID, and list of all available commands.
...
_File last updated: 2025-10-19 13:46:29 UTC_
