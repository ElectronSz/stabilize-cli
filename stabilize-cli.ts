...
/**
 * @module stabilize-cli
 * @description The CLI entrypoint for Stabilize ORM. Provides commands for generating models, migrations, seeds,
 * managing database state (migrate, rollback, seed, db:drop, db:reset), and status reporting.
 * 
 * @see https://github.com/ElectronSz/stabilize-cli
 */

import { program } from "commander";
import { generateMigration, runMigrations, Stabilize, defineModel, type Migration } from "stabilize-orm";
import { LogLevel, type DBConfig, type LoggerConfig, DBType } from "stabilize-orm";
...
/**
 * @constant C
 * @description ANSI escape codes for colored CLI output.
 */
const C = {
...
/**
 * @namespace CLILogger
 * @description Utility for colored CLI logging.
 */
const CLILogger = {
    /**
     * Log info message.
...
/**
 * @namespace spinner
 * @description CLI spinner utility for progress indication.
 */
const spinner = {
    chars: ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"],
    interval: 80,
...
/**
 * Load database config file and initialize ORM.
 * @param {string} configPath
 * @returns {Promise<{ config: DBConfig; orm: Stabilize }>}
 */
...
/**
 * Format SQL query for positional parameters based on DB type.
 * @param {string} query
 * @param {string} dbType
 * @returns {string}
 */
...
/**
 * Confirm prompt utility for destructive commands.
...
// --------------------------------------------------------------------------------------------------
/**
 * Perform topological sort of seed graph to resolve dependencies.
 * @param {Map<string, { file: string; dependencies: string[] }>} graph
 * @returns {string[]}
 */
...
/**
 * Normalize import paths for seed files.
 * @param {string} file
 * @returns {string}
 */
function toImportPath(file: string) {
